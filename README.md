...
1. 第一级热力学规则粗筛（统一决策内核 v2：全温域 25~650℃、12 条路径，给出排除/通过原因）；
...
## 统一决策内核 v2（2026-09-04 接线）

- 两级决策与 `03_程序代码/case_benchmark.py` 案例回溯共用同一 `decision_core.py`
  （主版本在项目 `11_决策内核v2_全温域/`；本目录为同步副本 + 适配层 `decision_v2_ui.py`）；
- 12 条路径：直接换热、余热锅炉产汽、吸收式（自驱动/外购蒸汽）、压缩式、
  ORC、高温蒸汽发电（蒸汽朗肯 280~650℃）、热化学/相变储热、TEG，
  及 v2.1 新增吸收式制冷（余热驱动）/ 电压缩制冷（电驱动基准）；
- 侧边栏“驱动来源”：余热自驱动 / 外购蒸汽 / 外购电力（外购蒸汽 + 供暖/热水
  启用哈石化同型“吸收式热泵提温（外购蒸汽）”路径）；
- 用能需求支持“供冷（制冷）”“干燥/烘干”：供冷按驱动来源严格区分、
  干燥按供热语义映射（除湿未单列）；
- **能力圈外不硬推荐**：>650℃ 发电/产汽提示“需多压/再热锅炉专项设计”；
  外购蒸汽驱动吸收式制冷不设废热回收路径（用能设备口径）。
- **S2 型边界工况自动提示（2026-09-05）**：热源温度接近“直接换热下限”
  （供暖需求温度+端差 / 干燥热风下限）或吸收式制冷 80~84℃ 驱动下限区时，
  第一级结果下方自动提示**预留 ≥5℃ 端差裕量**或按“直接换热优先 + 热泵兜底”
  双方案设计（对应鲁棒性报告 S2：75℃ 恰处可行边界，输入扰动下保持概率约 50%）。
...
waste-heat-demo/
├── app.py                  # 主程序
├── decision_core.py        # 统一决策内核 v2（同步副本）
├── decision_v2_ui.py       # 平台↔内核适配层
├── requirements.txt        # 依赖
