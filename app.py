...
RAW = {
    ...
    # 投资/回收期出处：ORC=《重庆大学学报》2019(23800元/kW、回收期5.58年)；吸收式热泵=660MW机组
    # 废热利用分析(1500万元/台、8~9年，按10MW级折算)；压缩式热泵=大庆油田供暖改造工程案例
    # (627万元/14.4MW、回收期1年，替代天然气锅炉)；直接换热/余热锅炉=工程估算(示意)；
    # 储热/TEG=示意(无公开统一报价或按文献下界折算)。政策分规则见《数据来源台账》：
    # 5分=国家层面重点推广/示范档(≤30%)；4分=目录余热利用类(≤20%)；3分=常规节能降碳/循环经济档(≤15%)；
    # 2分=未明确列入主要支持目录。
    "直接换热供暖":   [90,  60, 2.5, 0, 3],
    "余热锅炉直接产汽": [85, 100, 3.5, 0, 3],
    "吸收式热泵提温": [75, 150, 8.5, 0, 4],
    "压缩式热泵提温": [280, 44, 1.0, 0, 5],
    "ORC 余热发电":  [12.3, 2380, 5.6, 0, 4],
    "热化学储热":    [70, 900, 10.0, 0, 3],
    "相变储热":      [75, 600, 8.0, 0, 3],
    "TEG 热电发电":  [5, 1500, 12.0, 0, 2],
}
MAP_TO_TOPSIS = {
...
if survivors:
    ...
    st.caption("数据口径：能效列中 ORC 为 DWSIM 仿真中位热效率、其余为文献典型参考值；减排列按当前参数推算"
               "（替代购电/替代天然气），可复算；**投资/回收期**：ORC 引自《重庆大学学报》(23800元/kW、"
               "回收期5.58年)，压缩式热泵引自大庆油田供暖改造工程案例(627万元/14.4MW、回收期1年，"
               "替代天然气锅炉)，吸收式热泵引自 660MW 机组废热利用分析(1500万元/台、8~9年，按10MW级折算)，"
               "直接换热/余热锅炉为工程估算，储热与 TEG 为示意；**政策分**按《节能降碳中央预算内投资专项管理办法》"
               "（先进技术示范≤30%/节能降碳≤20%/循环经济≤15%）与《绿色低碳转型产业指导目录(2024年版)》"
               "收录情况打分，详见《数据来源台账》；权重为演示权重（AHP 矩阵为演示假设）。"
               "提示：当某路径在所有指标上均占优时，TOPSIS 贴近度会趋于 1（另一路径趋于 0），"
               "这是正常数学结果，非数据误差。")
